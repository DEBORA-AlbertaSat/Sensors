...
#
# ** Important note: Each command must be accompanied with a two digit hex checksum. It's 
# not that easy to understand how to find this but basically you need to convert each  
# character of a command/message '$' and '*' from ascii into hex. Then, consecutively XOR 
# the lower half bytes (nibble).Do the same for the upper half byte. The result of both 
# these operations is the checksum.
#
...
#
# Check Antenna Status (connected/disconnected): $PTNLQBA*54
#
...
# Outputs the response or acknowledgement from the GPS receiver
